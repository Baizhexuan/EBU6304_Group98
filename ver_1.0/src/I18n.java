...
public final class I18n {
    ...
    private static void addDashboards() {
        ...
        put("System Rules", "系统规则");
        put("Current System Rules", "当前系统规则");
        put("Workload limit:", "工作量上限:");
        put("Near-limit warning:", "接近上限预警:");
        put("Pre-approval message limit:", "同意对话前消息上限:");
        put("TA-MO pre-approval message limit:", "TA-MO 同意对话前消息上限:");
        put("Reputation penalty:", "信誉分惩罚:");
        put("Reputation penalty trigger:", "信誉分惩罚触发条件:");
        put("Reputation penalty amount:", "信誉分扣分:");
        put("Reputation default/floor:", "信誉分默认值/下限:");
        put("Above this value, Admin sees OVERLOAD.", "超过该值时，Admin 会看到超负荷。");
        put("At or above this value, Admin sees NEAR LIMIT.", "达到或超过该值时，Admin 会看到接近上限。");
        put("messages per sender before MO approval.", "条；MO 同意对话前每个发送方最多可发送这么多条。");
        put("original match >=", "原始匹配分 >=");
        put("and MO final rating <=", "且 MO 最终评分 <=");
        put("Future match score = base match * reputation / 100.", "未来匹配分 = 基础匹配分 * 信誉分 / 100。");
    ...
    private static void addValidationAndStatus() {
        ...
        put("Download CV PDF", "下载简历 PDF");
        put("Please select an applicant row first.", "请先选择一条申请者记录。");
        put("Application data is missing.", "申请数据缺失。");
        put("Applicant profile data is missing.", "申请者资料缺失。");
        put("CV Export Error", "简历导出错误");
        put("Overwrite Existing PDF", "覆盖已有 PDF");
        put("The selected PDF already exists. Replace it?", "所选 PDF 已存在，是否替换？");
        put("Original CV PDF exported to ", "原始简历 PDF 已导出至 ");
        put("Generated CV summary PDF exported to ", "简历摘要 PDF 已生成至 ");
        put("CV PDF exported to ", "简历 PDF 已导出至 ");
        put("Failed to export CV PDF: ", "导出简历 PDF 失败: ");
