...
public class FileStorage {
    ...
    public static void initDataFiles() {
        ...
        if (!usersFile.exists()) {
            try (PrintWriter pw = new PrintWriter(new FileWriter(usersFile))) {
                ...
                pw.println("2,ta1,ta123,TA");
                pw.println("3,ta2,ta456,TA");
                pw.println("4,mo1,mo123,MO");
                pw.println("5,mo2,mo456,MO");
            } catch (IOException e) {
    ...
    public static void saveUsers(List<User> users) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(DATA_DIR + "users.csv"))) {
            pw.println("id,username,password,role");
            for (User u : users) {
                pw.println(u.id + "," + sanitise(u.username) + "," + sanitise(u.password) + "," + sanitise(u.role));
            }
        } catch (IOException e) {
